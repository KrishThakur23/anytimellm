...
export default function UseCasesPage() {
  const cases = [
    {
      ...
      gradient: "from-rose-950/[0.08] via-transparent to-transparent hover:border-rose-500/20"
    },
    {
      ...
      gradient: "from-amber-950/[0.08] via-transparent to-transparent hover:border-amber-500/20"
    },
    {
      ...
      gradient: "from-emerald-950/[0.08] via-transparent to-transparent hover:border-emerald-500/20"
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#02000f] text-white relative flex flex-col justify-between overflow-x-hidden">
      ...
      {/* Background Glows */}
      <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-purple-600/[0.01] rounded-full blur-[120px] pointer-events-none" />
      ...
      <section className="relative pt-36 pb-12 px-6 md:px-12 max-w-7xl mx-auto w-full text-center z-10">
        <div className="mb-4 inline-flex items-center gap-1.5 border border-purple-500/20 bg-purple-950/20 px-3 py-1" style={{ borderRadius: '9999px' }}>
          <Star className="w-3.5 h-3.5 text-purple-300 animate-pulse fill-current" />
        ...
        <p className="font-body-lg text-lg text-text-secondary italic max-w-2xl mx-auto mb-10 leading-relaxed">
      ...
      <section className="py-8 px-6 md:px-12 max-w-7xl mx-auto w-full z-10 grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch mb-20">
        {cases.map((c, idx) => (
          <div
            key={idx}
            className={`glass-card-interactive flex flex-col justify-between p-8 relative transition-all duration-500 hover:scale-[1.01] bg-gradient-to-br ${c.gradient}`}
            style={{ borderRadius: 'var(--radius-xl)' }}
          >
            <div>
              {/* Header Title */}
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-purple-950/20 border border-purple-500/20 flex items-center justify-center" style={{ borderRadius: 'var(--radius-md)' }}>
                  {c.icon}
                </div>
                <h2 className="font-display-lg text-xl tracking-wider uppercase text-white font-bold">
                  {c.title}
                </h2>
              </div>

              {/* Tagline */}
              <span className="font-mono text-[8px] tracking-[0.2em] text-purple-400 uppercase block mb-3 font-bold">
                {c.tagline}
              </span>
              
              <p className="font-body-md text-xs text-text-secondary leading-relaxed mb-6">
                {c.description}
              </p>

              {/* Bullet points */}
              <ul className="space-y-3 mb-8 border-t border-border-subtle/50 pt-5">
                {c.bullets.map((bullet, bIdx) => (
                  <li key={bIdx} className="font-body-md text-xs text-text-secondary leading-normal flex items-start gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-purple-400/40 shrink-0 mt-1.5" />
                    <span>{bullet}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="border border-border-subtle bg-[#0b141a] p-4 font-sans text-xs select-none shadow-inner" style={{ borderRadius: 'var(--radius-lg)' }}>
              ...
              <div className="space-y-3 leading-normal">
                {/* Customer bubble */}
                <div className="bg-[#202c33] text-white/90 p-2.5 max-w-[85%] border border-white/5 shadow-sm" style={{ borderRadius: '0 10px 10px 10px' }}>
                ...
                <div className="bg-[#005c4b] text-white p-2.5 max-w-[85%] ml-auto border border-white/5 shadow-sm" style={{ borderRadius: '10px 0 10px 10px' }}>
