...
export default function PrivacyPolicyPage() {
  return (
    <div className="min-h-screen bg-white text-slate-900 relative flex flex-col justify-between overflow-x-hidden">
      ...
      {/* Background Glows */}
      <div className="absolute top-[10%] left-1/4 w-[500px] h-[500px] bg-purple-600/[0.02] rounded-full blur-[120px] pointer-events-none" />
      ...
      <section className="px-6 md:px-12 max-w-4xl mx-auto w-full z-10 mb-24 font-body text-xs md:text-sm text-slate-600 space-y-8 leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="p-5 border border-slate-200 bg-slate-50/50 hover:bg-white hover:border-slate-300 transition-all duration-300 rounded-2xl shadow-sm">
            <Lock className="w-5 h-5 text-purple-600 mb-3" />
            <h3 className="font-display text-xs uppercase tracking-wider text-slate-900 font-bold mb-2">Tenant Isolation</h3>
            <p className="text-[11px] text-slate-500">
              Your uploaded catalogs, menus, and business databases are strictly partitioned using multi-tenant namespaces.
            </p>
          </div>
          <div className="p-5 border border-slate-200 bg-slate-50/50 hover:bg-white hover:border-slate-300 transition-all duration-300 rounded-2xl shadow-sm">
            <Eye className="w-5 h-5 text-purple-600 mb-3" />
            <h3 className="font-display text-xs uppercase tracking-wider text-slate-900 font-bold mb-2">Message Encryption</h3>
            <p className="text-[11px] text-slate-500">
              Customer messages forwarded via WhatsApp Cloud API are processed on-the-fly and never sold to third parties.
            </p>
          </div>
          <div className="p-5 border border-slate-200 bg-slate-50/50 hover:bg-white hover:border-slate-300 transition-all duration-300 rounded-2xl shadow-sm">
            <Server className="w-5 h-5 text-purple-600 mb-3" />
            <h3 className="font-display text-xs uppercase tracking-wider text-slate-900 font-bold mb-2">Secure Storage</h3>
            <p className="text-[11px] text-slate-500">
